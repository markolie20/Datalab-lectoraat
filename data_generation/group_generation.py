...
from dotenv import load_dotenv # Not strictly needed here, but kept from original
import json
...
def generate_dynamic_providers(faker_instance):
    ...
    age_provider = DynamicProvider(provider_name='age_provider', elements=list(range(18, 90))) # Adjusted max age
    faker_instance.add_provider(age_provider)

def get_employment_status(age):
    if age < 18: # Should not happen with age_provider starting at 18
    ...
    elif age < 67: # Adjusted retirement age
        return random.choices(
...
def get_income_level(socio_economic_status, employment_status): # Renamed for clarity
...
def household_composition(age, marital_status, employment_status):
    if marital_status == 'Ongehuwd' or marital_status == 'Relatie':
        ...
        else: # 35+ and Unmarried/Relationship (less common to be with parents)
            return random.choices(['Alleenstaand', 'Samenwonend (met partner)', 'Alleenstaande ouder', 'Woongroep'], weights=[60, 20, 15, 5])[0]
    elif marital_status == 'Gehuwd' or marital_status == 'Samenwonend': # Samenwonend implies living together
        if age < 30:
...
def get_typical_online_hours(age, employment_status):
    ...
    if employment_status == 'Student':
        ...
        hours["weekdays"] = random.choice([
            ["09:00-17:00"], # Actively searching
            ["10:00-14:00", "19:00-22:00"], # More sporadic
        ])
    ...
    # Weekend patterns (generally more flexible)
    if age < 30:
    ...
    return hours  # Return as dict, not as str(hours)
...
def generate_people(faker_instance, num_people):
    ...
    for _ in range(num_people):
        person = faker_instance.profile(fields=['name', 'sex', 'job', 'birthdate']) # Added more fields
        ...
        typical_online_hours = get_typical_online_hours(age, employment_status)  # Already a dict now
        
        # Response latency influenced by age/tech
        ...
        if age > 60 or technology_proficiency in ['Zeer beperkt', 'Beginner']:
            ...
            if base_latency == 'Onmiddellijk': # Less likely for this group
                 base_latency = random.choice(latency_options)
        response_latency_profile = base_latency

        # Emoji usage influenced by age/tech
        ...
        # Punctuation, Spelling, Grammar influenced by education
        ...
        person.update({
            ...
            'income_level': income, # Renamed from income
            'age': age,
            # New chat behavior attributes
        ...
        # Remove some less relevant default profile fields if not needed for bio generation
        # For now, keeping them as they might provide some flavor
...
# generate_person function was not used in the original main, so commenting out for now
...
def main():
    ...
    Faker.seed(313) # Seed Faker globally
    random.seed(313) # Seed random module
    
    GROUP_DATA_FOLDER = "data/groups" # Changed folder name
    GROUP_AMOUNT = 1000 # Reduced for faster testing, was 1000
    MIN_PEOPLE = 10  # Was 10
    MAX_PEOPLE = 35 # Was 35
    ...
    generate_dynamic_providers(faker_instance) # Call this once
    ...
    for group_id, group_size in enumerate(people_per_group):
        ...
        people_df.to_csv(filepath, index=False, encoding='utf-8-sig') # ensure utf-8 for special chars
